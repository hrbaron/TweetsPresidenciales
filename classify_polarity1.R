classify_polarity <- function(textColumns,algorithm="e1071",pstrong=0.5,pweak=1.0,prior=1.0,verbose=TRUE,...) {
  
  sink("output.txt")
  matrix <- create_matrix(textColumns,...)
  #lexicon <- read.csv("C:/Users/hbaronfr/Desktop/CONEXION/subjectivitynorepeated.csv",header=FALSE)
  lexicon <- read.csv("C:/Tweets_Candidatos/subjectivitynorepeated.csv",header=FALSE)
	...
	for (i in 1:nrow(matrix)) {
		...
		for (word in words) {
			...
			if (index > 0) {
				...
				if (algorithm=="e1071") score <- abs(log(score*prior/count))
	...
	print(paste("Positivos: ",compos,"Negativos: ",comneg,"Negativos: ",comneu))
	cat("\n")
	sink()
